...
import 'package:timetable/screens/view_nd2_second.dart';
...
class ND2Add_second extends StatefulWidget {
  const ND2Add_second({super.key});

  @override
  State<ND2Add_second> createState() => _ND2Add_secondState();
}

class _ND2Add_secondState extends State<ND2Add_second> {
  ...
  void saveData() async {
    ...
    if (snap2.length < 11) {
      String res = await AuthMethods().saveND2second(
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        title: const Text(
          'ADD COURSE/LECTURER 200L SECOND',
                ...
                child: ElevatedButton(
                  onPressed: () {
                    Navigator.of(context).pushReplacement(
                      MaterialPageRoute(
                        builder: (context) => const ViewNd2Second(),
                      ),
                    );
                  },
                  style: ButtonStyle(
